/**
 * Script to check fireStations data coverage
 * 
 * This script checks what fire stations data exists
 * and their geographic coverage.
 * 
 * Usage: Run this script in MongoDB Compass shell
 */

// Function to check fire stations data
function checkFireStationsData() {
    print("🔍 Checking fireStations data...");
    ...
    sampleDocs.forEach((doc, index) => {
        ...
        print(`      Phone: ${doc.properties?.EmergencyPhoneNumber || 'N/A'}`);
        print("");
    });
...
function testCoordinateFormats() {
    ...
    testPoints.forEach(point => {
        ...
        const result = db.getCollection('fireStations').aggregate([
            {
                $geoNear: {
...
function main() {
    print("🚀 Starting fireStations data analysis...");
    print("=" * 50);

    try {
        checkFireStationsData();
        testCoordinateFormats();

        print("\n🎉 FireStations data analysis completed!");
        print("=" * 50);
