...
class SafetyMonitorService {
    ...
    async createOrMergeClaimForIncident(incident) {
        ...
        // Get project address for default eventAddress
        let projectAddress = '';
        if (projectId) {
            try {
                const proj = await projects.findOne({ _id: new ObjectId(projectId) });
                projectAddress = proj?.city || proj?.address || proj?.projectName || '';
            } catch (e) {
                console.warn('⚠️ Failed to get project address:', e.message);
            }
        }
        ...
        let thumbnailUrl = '';
        try {
            console.log('🖼️ Generating PDF thumbnail for:', incident.reportLink);
            thumbnailUrl = await this.generatePdfThumbnail(incident.reportLink) || '';
            if (thumbnailUrl) {
                console.log('✅ PDF thumbnail generated:', thumbnailUrl);
            } else {
                console.warn('⚠️ Failed to generate PDF thumbnail, using fallback');
            }
        } catch (e) {
            ...
            generalAttachments: [
                {
                    ...
                    thumbnailUrl: thumbnailUrl || incident.reportLink, // Use generated thumbnail or fallback to PDF URL
                    validUntil: '', // Will be set to email date later
    ...
    async generatePdfThumbnail(pdfUrl) {
        try {
            ...
            if (!response.ok) {
                console.warn('⚠️ Failed to generate PDF thumbnail:', response.statusText);
                return null;
            }
            ...
            console.warn('⚠️ Error generating PDF thumbnail:', error.message);
            return null;
        }
    }
    ...
    async ensureAttachment(claimId, url, description = '') {
        ...
        if (!exists) {
            ...
            let thumbnailUrl = '';
            try {
                console.log('🖼️ Generating PDF thumbnail for attachment:', url);
                thumbnailUrl = await this.generatePdfThumbnail(url) || '';
                if (thumbnailUrl) {
                    console.log('✅ PDF thumbnail generated:', thumbnailUrl);
                } else {
                    console.warn('⚠️ Failed to generate PDF thumbnail, using fallback');
            ...
            list.push({
                ...
                fileUrl: url,
                thumbnailUrl: thumbnailUrl || url, // Use generated thumbnail or fallback to PDF URL
                validUntil: '', // Will be set to email date later
                isAutoGenerated: true // Mark as auto-generated to hide delete button
            });
