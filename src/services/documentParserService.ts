...
export class DocumentParserService {
   ...
   */
  static async parseSoilReport(fileUrl: string): Promise<SoilReportData> {
    try {
   ...
   */
  private static async extractTextFromDocument(fileUrl: string): Promise<string> {
    try {
   ...
   */
  private static async extractTextFromImage(fileUrl: string): Promise<string> {
    try {
      const response = await openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
   ...
   */
  private static async extractDataWithOpenAI(documentText: string): Promise<SoilReportData> {
    try {
      const prompt = `
...
`;
   ...
   */
  private static validateAndCleanData(data: any): SoilReportData {
    const cleaned: SoilReportData = {};
